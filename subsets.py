...
for index1 in range(1, len(trips)+1):
    ...
    mainTrip = trips[index1]
    for index2 in range(index1+1, len(trips)+1):
        # secondaryTrip = " ".join(map(str,trips[index2]))
        secondaryTrip = trips[index2]
        # if secondaryTrip in mainTrip: #print set(podiekkena).issubset(lokuekkena)
        if set(secondaryTrip).issubset(mainTrip): #print set(podiekkena).issubset(lokuekkena)
            subsetOf[index1].append(index2)
            parents[index2].append(index1)
            print index2, "is a subset of", index1
        elif set(mainTrip).issubset(secondaryTrip):
...
outfile1 = open("subsetResults/subsets2.txt", "w")
for key in subsetOf:
...
outfile2 = open("subsetResults/parents2.txt", "w")
for key in parents:
...
outfile3 = open("subsetResults/subsetcount2.txt", "w")
...
outfile4 = open("subsetResults/parentcount2.txt", "w")
...
print "sorted trips according to child node count"
